<template>
  <div>
    <div>
      <el-form :inline="true" :model="formInlineRole" ref="formInlineRole" class="demo-form-inline searchStyle">
        <el-form-item label="名称"  prop="roleName"> <!--prop 重置时用到  -->
          <el-input v-model="formInlineRole.roleName" clearable></el-input>
        </el-form-item>
        <el-form-item>
          <el-button type="primary" @click="searchFormRole()">查询</el-button>
        </el-form-item>
         <el-form-item>
          <el-button @click="resetFormRole('formInlineRole')">重置</el-button>
        </el-form-item>
      </el-form>
    </div>
    <div>
    ...
    <!-- 新增角色 -->
      <el-dialog title="新增角色" :visible.sync="dialogAdd" width="30%" :close-on-click-modal="false" class="roleDialog">
      ...
      <!-- 修改角色 -->
      <el-dialog title="修改角色" :visible.sync="dialogEdit" width="30%" :close-on-click-modal="false" class="roleDialog" :before-close="cancelDialog">
       ...
       <!-- 用户授权 -->
      <el-dialog :title='"用户授权-"+titleUserAuthorize' :visible.sync="dialogUserAuthorize" width="50%" :close-on-click-modal="false" :before-close="cancelDialog">
        <div>
...
export default {
    data (){
       //表单验证
        ...
        return{
          ...
          delUrl: this.$store.state.url+'webauth/role/deleteRole',//删除
          dataSize: 0, //总数
          tableData: [],//表格数据
          pageNum: 1,//第几页
          ...
          /*用户授权数据起*/
          getUserDataUrl: this.$store.state.url+'webauth/user/listUsers',//获取用户表格数据
          getHasAssgin:this.$store.state.url+'webauth/role/listRoleUsersCurrentPage',//获取当前页用户 已有授权的用户
          userChange: this.$store.state.url+'webauth/role/saveRoleUsers',//用户授权确认
          dataSize2: 0, //总数
          tableData2: [],//表格数据
          pageNum2: 1,//第几页
          ...
          getHasBottonUrl:this.$store.state.url+'webauth/role/listRoleMenuBtns',//查询已有按钮
          roleChangeUrl:this.$store.state.url+'webauth/role/saveRoleMenuBtn',//角色授权确认
    ...
    methods: {
      ...
      //重置查询域
      resetFormRole(formInlineRole){
        this.$refs[formInlineRole].resetFields();
      },
      //查询-用户授权 
      searchForm(){
        var params = new URLSearchParams();
      ...
      getData() {
        var params = new URLSearchParams();
        params.append('roleName', this.formInlineRole.roleName);
     ...
     // 获取该角色已有按钮
      ...
      getUseHasChoose(){
        ...
        this.pageRoleCodes=userCodes;
        var params = new URLSearchParams();
            params.append('roleCode', this.editInfoBefore.roleCode);
            params.append('userCodesPage', this.pageRoleCodes);
            ...
            if(0==res.data.code){
             ...
             this.selectId2 = userCode;//将选中的id存起来
            }
            })
      },
      ...
      getData2() {
        ...
        if(0==res.data.code){//查询成功
          ...
          this.dataSize2=res.data.data.total;
      ...
      userAuthorizeSure(){
        var params = new URLSearchParams();
        params.append('roleCode', this.editInfoBefore.roleCode);//角色code
        params.append('userCodesPage', this.pageRoleCodes );//当前页用户
        params.append('userCodesChecked',  this.selectId2);//已经勾选用户
        this.openFullScreen();//打开遮罩
      ...
      menuAuthorizeSure(){
        ...
        var params = new URLSearchParams();
        params.append('roleCode', this.editInfoBefore.roleCode);
        params.append('menuCodes', select);
        params.append('menuBtn', this.newChoosenBtn);
        this.openFullScreen();//打开遮罩
...
<style>
.roleDialog .el-form-item__error {
  padding-top: 0;
}

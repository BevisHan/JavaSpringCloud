       ...
       params.append('authCode',val);
        this.$axios.post(this.url,params).then(res =>  {
            if(0==res.data.code){
                ...
                // if(""!=flag){//首页工作台进来的  先改变路由 然后 修改头部导航的值 根据路由名称 从查询出来的菜单中找 区分一级二级
                //    for(var i=0;i<this.items.length;i++){
                //       if(this.items[i].children){
                //           for(var j=0;j<this.items[i].children.length;j++){
                //             if(flag==this.items[i].children[j].indexRoute){
                //               this.$store.state.subMenuName.push(this.items[i].name)
                //               this.$store.state.subMenuName.push(this.items[i].children[j].name)
                //               this.$store.state.menuCode=this.items[i].children[j].id;
                //             }
                //           }
                //       }else{
                //         if(flag==this.items[i].indexRoute){
                //           this.$store.state.subMenuName.push(this.items[i].name)
                //           this.$store.state.menuCode=this.items[i].id;
                //         }
                //       }
                //     }
                //   //  this.$router.push({ path: '/'+ flag});
                // }else{
                //    if(this.$route.query.searchData&&this.$route.query.menuId){//此处是新开浏览器的设置
                //       this.$router.push({
                //           query: {
                //           }
                //       });
                ...
                //       this.$store.state.menuCode=this.items[0].children[0].id;//给菜单code放到全局变量
                //       this.$router.push({ path: '/'+this.items[0].children[0].indexRoute});
                //       this.$store.state.subMenuName.push(this.items[0].name)
                //       this.$store.state.subMenuName.push(this.items[0].children[0].name)
                //     }else{
                //       this.$store.state.menuCode=this.items[0].id;//给菜单code放到全局变量
                //       this.$router.push({ path: '/'+this.items[0].indexRoute });
                //       this.$store.state.subMenuName.push(this.items[0].name)
                //    }
                //    }
                // }

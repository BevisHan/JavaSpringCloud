<template>
    <div class="header">
        ...
        <!-- 修改密码 -->
        <el-dialog title="修改密码" :visible.sync="dialogChangePsw" width="25%" :close-on-click-modal="false">
            <el-form :model="changePswList" :rules="rulesChangePsw" ref="changePswList" label-width="100px">
    ...
    export default {
        ...
        methods:{
                ...
                this.$axios.post(this.getFirstleave,params).then(res =>  {
                    if(0==res.data.code){
                        ...
                        if(this.$route.query.searchData&&this.$route.query.menuId){//此处是新开浏览器的设置
                            list.label=this.$route.query.menuName;
                            list.name=Number(this.$route.query.menuId);
                           // this.activeName=Number(this.$route.query.menuId);//默认选中一级菜单第一个
                        }else{
                            list.label=this.firstLeave[0].label
                            list.name=this.firstLeave[0].id
                            this.activeName=this.firstLeave[0].id;//默认选中一级菜单第一个
                        }
                        //  let list={
                        //         label:this.firstLeave[0].name,
            ...
            chooseFirstLeave(tab, event) {
                ...
                this.$store.state.subMenuName.push(tab.label)

<template>
    ...
    <el-table 
       ...
       <!--  ref="multipleTable" -->
        ...
        <el-table-column prop="gmtCreate" label="创建时间" :formatter="dateFormat">
        </el-table-column>
        <el-table-column prop="gmtModified" label="修改时间" :formatter="dateFormat">
        </el-table-column>
    ...
    <el-dialog width="52%" title="修改用户" :visible.sync="dialogEdit" :close-on-click-modal="false" class="userDialog" :before-close="cancelDialog">
    ...
    <!-- 修改密码 -->
    <el-dialog title="修改密码" :visible.sync="dialogPwdEdit" width="30%" :close-on-click-modal="false" class="changePassword" :before-close="cancelDialog">
      ...
      <div style="padding-left:60px">
        ...
        <div style="width:251px;float:left">
          <el-checkbox-group v-model="roleForm.type" @change="chooseCheck()"> 
            <div v-for="item in roleFormData"  :key="item.roleCode" class="tdStyle rightNoStyle topNoStyle">
              <el-checkbox :label="item.roleCode"><span></span></el-checkbox>
            </div>
          </el-checkbox-group>
        </div>
        <div style="width:251px;float:left">
          <div v-for="item in roleFormData" :key="item.roleCode"  class="tdStyle topNoStyle">
    ...
    <!--设置岗位 -->
    <el-dialog :title='"设置岗位-"+titleUserName' :visible.sync="dialogPostSet"  :close-on-click-modal="false" :before-close="cancelDialog">
      <div>
        <el-form :inline="true" :model="formInlinePost" ref="formInlinePost" class="demo-form-inline searchStyle">
          <el-form-item label="岗位名称"  prop="postName"> <!--prop 重置时用到  -->
            <el-input v-model="formInlinePost.postName" clearable></el-input>
          </el-form-item>
          <el-form-item>
            <el-button type="primary" @click="searchFormPost()">查询</el-button>
          </el-form-item>
          <el-form-item>
            <el-button @click="resetFormPost('formInlinePost')">重置</el-button>
          </el-form-item>
        </el-form>
      </div>
      <el-table 
...
export default {
  ...
  methods: {
    ...
    getData() {
        // var list=[{
        //   createBy: "0",
        //   email: "he_yzh@neusoft.com",
        //   gmtCreate: 1552290951000,
        //   gmtModified: 1552293097000,
        //   id: "ce808663c57a4e9da52bc127cc688b59",
        //   idCard: "0",
        //   isAdmin: "否",
        //   isDeleted: 0,
        //   lastModifiedBy: "0",
        ...
        console.log(res.data.data.list)
          ...
          for(var i=0;i<this.tableData.length;i++){
              ...
              if(0==this.tableData[i].isAdmin){
                  this.tableData[i].isAdmin='否'
              }else{
                  this.tableData[i].isAdmin='是'
              }
          }
